...
SuspectFlow::SuspectFlow(uint8_t mod, uint64_t vm_id, uint64_t dp_id, uint32_t vm_port, std::vector<Match> matches, string ip_src, string gateway) {
    set_mod(mod);
    set_vm_id(vm_id);
    set_dp_id(dp_id);
    set_vm_port(vm_port);
...
int SuspectFlow::get_type() {
    return SUSPECT_FLOW;
}

uint8_t SuspectFlow::get_mod() {
    return this->mod;
}

void SuspectFlow::set_mod(uint8_t mod) {
    this->mod = mod;
}

uint64_t SuspectFlow::get_vm_id() {
    return this->vm_id;
}

void SuspectFlow::set_vm_id(uint64_t vm_id) {
    this->vm_id = vm_id;
}

uint64_t SuspectFlow::get_dp_id() {
    return this->dp_id;
}

void SuspectFlow::set_dp_id(uint64_t dp_id) {
    this->dp_id = dp_id;
}

uint32_t SuspectFlow::get_vm_port() {
    return this->vm_port;
}

void SuspectFlow::set_vm_port(uint32_t vm_port) {
    this->vm_port = vm_port;
}

std::vector<Match> SuspectFlow::get_matches() {
    return this->matches;
}

void SuspectFlow::set_matches(std::vector<Match> matches) {
    this->matches = matches;
}

void SuspectFlow::add_match(const Match& match) {
    this->matches.push_back(match);
}

string SuspectFlow::get_ip_src() {
    return this->ip_src;
}

void SuspectFlow::set_ip_src(string ip_src) {
    this->ip_src = ip_src;
}

string SuspectFlow::get_gateway() {
    return this->gateway;
}

void SuspectFlow::set_gateway(string gateway) {
    this->gateway = gateway;
}

void SuspectFlow::from_BSON(const char* data) {
    mongo::BSONObj obj(data);
    set_mod(string_to<uint8_t>(obj["mod"].String()));
    set_vm_id(string_to<uint64_t>(obj["vm_id"].String()));
    set_dp_id(string_to<uint64_t>(obj["dp_id"].String()));
    set_vm_port(string_to<uint32_t>(obj["vm_port"].String()));
    set_matches(MatchList::to_vector(obj["matches"].Array()));
    set_ip_src(obj["ip_src"].String());
    set_gateway(obj["gateway"].String());
}

const char* SuspectFlow::to_BSON() {
    mongo::BSONObjBuilder _b;
    _b.append("mod", to_string<uint16_t>(get_mod()));
    _b.append("vm_id", to_string<uint64_t>(get_vm_id()));
    _b.append("dp_id", to_string<uint64_t>(get_dp_id()));
    _b.append("vm_port", to_string<uint32_t>(get_vm_port()));
    _b.appendArray("matches", MatchList::to_BSON(get_matches()));
    _b.append("ip_src", get_ip_src());
    _b.append("gateway", get_gateway());
    mongo::BSONObj o = _b.obj();
    char* data = new char[o.objsize()];
    memcpy(data, o.objdata(), o.objsize());
    return data;
...
string SuspectFlow::str() {
    ...
    ss << "  vm_port: " << to_string<uint32_t>(get_vm_port()) << endl;
    ss << "  matches: " << MatchList::to_BSON(get_matches()) << endl;
    ss << "  ip_src: " << get_ip_src() << endl;
    ss << "  gateway: " << get_gateway() << endl;
    return ss.str();
}

IptablesRegister::IptablesRegister() {
    set_vm_id(0);
    set_vm_port(0);
    set_hwaddress(MACAddress());
    set_blocked_ip("");
...
IptablesRegister::IptablesRegister(uint64_t vm_id, uint32_t vm_port, MACAddress hwaddress, string blocked_ip) {
    set_vm_id(vm_id);
    set_vm_port(vm_port);
    set_hwaddress(hwaddress);
    set_blocked_ip(blocked_ip);
}

int IptablesRegister::get_type() {
    return IPTABLES_REGISTER;
}

uint64_t IptablesRegister::get_vm_id() {
    return this->vm_id;
}

void IptablesRegister::set_vm_id(uint64_t vm_id) {
    this->vm_id = vm_id;
}

uint32_t IptablesRegister::get_vm_port() {
    return this->vm_port;
}

void IptablesRegister::set_vm_port(uint32_t vm_port) {
    this->vm_port = vm_port;
}

MACAddress IptablesRegister::get_hwaddress() {
    return this->hwaddress;
}

void IptablesRegister::set_hwaddress(MACAddress hwaddress) {
    this->hwaddress = hwaddress;
}

string IptablesRegister::get_blocked_ip() {
    return this->blocked_ip;
}

void IptablesRegister::set_blocked_ip(string blocked_ip) {
    this->blocked_ip = blocked_ip;
}

void IptablesRegister::from_BSON(const char* data) {
    mongo::BSONObj obj(data);
    set_vm_id(string_to<uint64_t>(obj["vm_id"].String()));
    set_vm_port(string_to<uint32_t>(obj["vm_port"].String()));
    set_hwaddress(MACAddress(obj["hwaddress"].String()));
    set_blocked_ip(obj["blocked_ip"].String());
}

const char* IptablesRegister::to_BSON() {
    mongo::BSONObjBuilder _b;
    _b.append("vm_id", to_string<uint64_t>(get_vm_id()));
    _b.append("vm_port", to_string<uint32_t>(get_vm_port()));
    _b.append("hwaddress", get_hwaddress().toString());
    _b.append("blocked_ip", get_blocked_ip());
    mongo::BSONObj o = _b.obj();
    char* data = new char[o.objsize()];
    memcpy(data, o.objdata(), o.objsize());
    return data;
...
string IptablesRegister::str() {
    stringstream ss;
    ss << "IptablesRegister" << endl;
    ss << "  vm_id: " << to_string<uint64_t>(get_vm_id()) << endl;
    ss << "  vm_port: " << to_string<uint32_t>(get_vm_port()) << endl;
    ss << "  hwaddress: " << get_hwaddress().toString() << endl;
    ss << "  blocked_ip: " << get_blocked_ip() << endl;
    return ss.str();
}

IptablesRegisterDynamic::IptablesRegisterDynamic() {
    set_vm_id(0);
    set_vm_port(0);
    set_blocked_ip("");
}

IptablesRegisterDynamic::IptablesRegisterDynamic(uint64_t vm_id, uint32_t vm_port, string blocked_ip) {
    set_vm_id(vm_id);
    set_vm_port(vm_port);
    set_blocked_ip(blocked_ip);
}

int IptablesRegisterDynamic::get_type() {
    return IPTABLES_REGISTER_DYNAMIC;
}

uint64_t IptablesRegisterDynamic::get_vm_id() {
    return this->vm_id;
}

void IptablesRegisterDynamic::set_vm_id(uint64_t vm_id) {
    this->vm_id = vm_id;
}

uint32_t IptablesRegisterDynamic::get_vm_port() {
    return this->vm_port;
}

void IptablesRegisterDynamic::set_vm_port(uint32_t vm_port) {
    this->vm_port = vm_port;
}

string IptablesRegisterDynamic::get_blocked_ip() {
    return this->blocked_ip;
}

void IptablesRegisterDynamic::set_blocked_ip(string blocked_ip) {
    this->blocked_ip = blocked_ip;
}

void IptablesRegisterDynamic::from_BSON(const char* data) {
    mongo::BSONObj obj(data);
    set_vm_id(string_to<uint64_t>(obj["vm_id"].String()));
    set_vm_port(string_to<uint32_t>(obj["vm_port"].String()));
    set_blocked_ip(obj["blocked_ip"].String());
...
const char* IptablesRegisterDynamic::to_BSON() {
    mongo::BSONObjBuilder _b;
    _b.append("vm_id", to_string<uint64_t>(get_vm_id()));
    _b.append("vm_port", to_string<uint32_t>(get_vm_port()));
    _b.append("blocked_ip", get_blocked_ip());
...
string IptablesRegisterDynamic::str() {
    stringstream ss;
    ss << "IptablesRegisterDynamic" << endl;
    ss << "  vm_id: " << to_string<uint64_t>(get_vm_id()) << endl;
    ss << "  vm_port: " << to_string<uint32_t>(get_vm_port()) << endl;
    ss << "  blocked_ip: " << get_blocked_ip() << endl;
    return ss.str();
