...
boost::thread FlowTable::DynamicIP;
...
void FlowTable::start(uint64_t vm_id, map<string, Interface> interfaces,
    ...
    DynamicIP = boost::thread(&FlowTable::DynamicIPMethod);
...
void FlowTable::DynamicIPMethod(){
   boost::posix_time::seconds worktime(20);
   while(true){
       std::cout  << "verifica se há iptables" << endl;
       boost::this_thread::sleep(worktime);
       
      system("iptables -n -L | egrep \"DROP|REJECT\" | awk '{print $4, $5}' | sed \"s/0.0.0.0\\/0//g\" | sed \"s/ //g\" > /root/blocked_dynamic_ip.txt");
      
      FILE *f2 = fopen("/root/blocked_ip.txt", "r");
      FILE *f = fopen("/root/blocked_dynamic_ip.txt", "r");
      if (f != NULL) {
         size_t len = 15; 
         char *linha = NULL;
         char *linha2 = NULL;
         ...
         while (getline(&linha, &len, f) > 0){
          ...
          if (!existe){
              IptablesRegisterDynamic msgIptablesDynamic(FlowTable::vm_id, 1, linha);
              FlowTable::ipc->send(RFCLIENT_RFSERVER_CHANNEL, RFSERVER_ID, msgIptablesDynamic);
          }
        }
        fclose(f);
        fclose(f2);
        remove("/root/blocked_dynamic_ip.txt");
     }
   }
}
...
int FlowTable::sendToHw(RouteModType mod, const RouteEntry& re) {
    ...
    if (mod == RMT_DELETE || mod ==RMT_REMOVE) {
...
int FlowTable::sendToHw(RouteModType mod, const IPAddress& addr,
   ...
   if (is_port_down(local_iface.port)) {
    ...
    }else if (mod == RMT_REMOVE){
        SuspectFlow sf;
        sf.set_vm_id(FlowTable::vm_id);
        sf.set_mod(mod);
        sf.set_ip_src(addr.toString());
        sf.set_gateway(mask.toString());
        sf.set_vm_port(local_iface.port);
        sf.add_match(Match(RFMT_IPV4, addr, mask));
        FlowTable::ipc->send(RFCLIENT_RFSERVER_CHANNEL, RFSERVER_ID, sf);
     }
    return 0;
}
