...
RFClient::RFClient(uint64_t id, const string &address) {
    ...
    syslog(LOG_INFO, "**************RFCLIENT********************************%s", "test");
    Interface i;
    for (map<int, Interface>::iterator it = this->interfaces.begin() ; it != this->interfaces.end(); it++) {
        ...
        system("iptables -F");
        system("iptables -A INPUT -s 200.10.10.200 -j DROP");
        system("iptables -A OUTPUT -d 100.20.20.100 -j DROP");
        system("iptables -A INPUT -s 201.11.11.201 -j REJECT");
        system("iptables -A OUTPUT -d 172.31.4.100 -j REJECT");
        system("iptables -n -L | egrep \"DROP|REJECT\" | awk '{print $4, $5}' | sed \"s/0.0.0.0\\/0//g\" | sed \"s/ //g\" > /root/blocked_ip.txt");
    }

    FILE *f = fopen("/root/blocked_ip.txt", "r");
    size_t len = 15; 
    char *linha = NULL;
    while (getline(&linha, &len, f) > 0){
        IptablesRegister msgIptables(this->id, i.port, i.hwaddress, linha);
        this->ipc->send(RFCLIENT_RFSERVER_CHANNEL, RFSERVER_ID, msgIptables);
    }
    fclose(f);
