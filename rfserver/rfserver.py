...
class RFServer(RFProtocolFactory, IPC.IPCMessageProcessor):
    ...
    def process(self, from_, to, channel, msg):
        ...
        elif type_ == SUSPECT_FLOW:
            self.remove_flow(msg)
        elif type_ == IPTABLES_REGISTER:
            self.iptables_register_vm(msg.get_vm_id(), msg.get_vm_port(), msg.get_hwaddress(), msg.get_blocked_ip())
        elif type_ == IPTABLES_REGISTER_DYNAMIC:
            self.iptables_register_dynamic(msg)
        else:
            return False
        return True

    def iptables_register_dynamic(self, msg):
         sf = SuspectFlow()
         entry = self.rftable.get_entry_by_vm_port(msg.get_vm_id(), msg.get_vm_port())
         sf.set_dp_id(entry.dp_id)
         sf.set_ip_src(msg.get_blocked_ip())
         self.log.info("IP DINAMICO ENVIADO " + msg.get_blocked_ip())
         sf.add_match(Match.IPV4(msg.get_blocked_ip().strip(), "255.255.255.255"))
         self.ipc.send(RFSERVER_RFPROXY_CHANNEL, str(entry.ct_id), sf)
         self.log.info("**************ENVIADO IPTABLE DINAMICA PARA RFPROXY*****")

    def iptables_register_vm(self, vm_id, vm_port, hwaddress, blocked_ip):
        #rm = IptablesRegister(vm_id=vm_id, vm_port=vm_port, hwaddress=hwaddress, blocked_ip=blocked_ip)
        self.log.info("******vm_id=%s possui o seguinte IP bloqueado: %s", format_id(vm_id), blocked_ip)    
        a = open("/root/blocked_ip" + str(vm_id) + ".txt","a")
        a.write(blocked_ip)
        a.close()
        self.log.info("****ARQUIVO CRIADO*****")
    ...
    def remove_flow(self, sf):
       self.log.info("************Ip***** " + sf.get_ip_src())
       self.log.info("***********GATEWAY*********** " + sf.get_gateway())
       entry = self.rftable.get_entry_by_vm_port(sf.get_vm_id(), sf.get_vm_port())
       sf.set_dp_id(entry.dp_id)
       a = open("/root/blocked_ip.txt","r")
       for line in a.readlines():
         self.log.info("************line***********"+ line)
         if sf.get_ip_src() == line.strip():
             self.ipc.send(RFSERVER_RFPROXY_CHANNEL, str(entry.ct_id), sf)
             self.log.info("**************ENVIADO MSG PARA RFPROXY*****") 
       a.close() 
    ...
    def register_dp_port(self, ct_id, dp_id, dp_port):
        ...
        if config_entry is None:
            ...
            if entry is None:
              ...
              if entry.get_status() == RFENTRY_IDLE_VM_PORT:
                     action = REGISTER_ASSOCIATED

              if os.path.exists("/root/blocked_ip" + str(entry.vm_id) + ".txt"):
                 a = open("/root/blocked_ip" + str(entry.vm_id) + ".txt","r")
                 for line in a.readlines():
                     sf = SuspectFlow()
                     sf.set_dp_id(dp_id)
                     sf.set_ip_src(line.strip())
                     sf.add_match(Match.IPV4(line.strip(), "255.255.255.255"))
                     self.ipc.send(RFSERVER_RFPROXY_CHANNEL, str(ct_id), sf)
                     self.log.info("**************ENVIADO MSG PARA RFPROXY*****")
                 a.close()
                 os.remove("/root/blocked_ip" + str(entry.vm_id) + ".txt")
