...
class RFProcessor(IPC.IPCMessageProcessor):
    def process(self, from_, to, channel, msg):
        ...
        if type_ == SUSPECT_FLOW:
            try:
                log.info("****MSG CHEGOU NO RFPROXY********* dp_id=" + str(msg.get_dp_id()) )
                ofmsg = create_flow_mod_remove(msg)
            except Warning as e:
                log.info("Error creating FlowMod: {}" % str(e))
            if send_of_msg(msg.get_dp_id(), ofmsg) == SUCCESS:
                log.info("suspect flow sent to datapath")
            else: 
                log.info("Error sending suspect flow to datapath")
        if type_ == IPTABLES_REGISTER:
            log.info("*****************************!!!!!!!IPTABLES REGISTER")
        return True
