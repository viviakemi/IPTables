...
def create_flow_mod_remove(suspectflow):
    log.info("*******RFOFMSG*******" + str(suspectflow))
    ofm = ofp_flow_mod()
    ofm.command = OFPFC_ADD
    ofm.priority = 40000 #Option.PRIORITY(PRIORITY_HIGHEST)
    ofm.actions.append(ofp_action_output(port=(OFPP_NONE & 0xFFFF)))
    #ofm.flags=OFPFF_SEND_FLOW_REM
    for match in suspectflow.get_matches():
